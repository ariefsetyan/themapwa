...
function showData()
{
	...
	if ( dataCustomer.length != 0 )
	{
		for ( i = 0; i < 3; i++ )
		{
			...
			dataXLS += "{\"ktp\":\"" + dataCustomer[i].ktp + "\",\"nama\":\"" + dataCustomer[i].nama + "\",\"alamat\":\"" + dataCustomer[i].alamat + "\"},";
	...
	dataXLS = dataXLS.substr(0,dataXLS.length-1);
	dataXLS += "]";
...
{
	dataXLS = JSON.parse(dataXLS);
	
	dirname = "ExcelAPI";
	filename = "file-latihan.xls";
	sheetname = "Data Customer";

	window.xls.save( dataXLS, dirname, filename, sheetname, 
